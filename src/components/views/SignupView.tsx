...
export const SignupView: React.FC<{ onSwitchToLogin: () => void }> = ({ onSwitchToLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // If email confirmations are off in Supabase Auth settings, the session
    // is already active after signUp; if they're on, this sign-in attempt
    // will fail until the user confirms — either way, tell them what to do.
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#FFFBF5] px-4">
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="bg-white border border-[#E7D9C9] rounded-2xl p-6 space-y-4 warm-shadow-lg">
          <h1 className="text-lg font-semibold text-[#1C110E]">Create your account</h1>
          ...
          {info && <p className="text-xs text-[#6E5B52]">{info}</p>}
          ...
          <p className="text-xs text-[#6E5B52] text-center">
            ...
            <button type="button" onClick={onSwitchToLogin} className="text-[#8E1B1B] font-medium underline">
              Sign in
            </button>
          </p>
        </form>
