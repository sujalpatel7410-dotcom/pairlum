  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 max-w-7xl mx-auto pb-20">
      ...
      >
        ...
        <div className="p-4 rounded-2xl bg-[#FFB8CB] border border-[#F4A9BF] space-y-3">
          <div className="flex items-center justify-between text-xs">
            <span className="font-semibold text-[#4A0420] flex items-center gap-1.5">
              <span className="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse" />
              <span>Partner Live</span>
            </span>
            <span className="text-[10px] text-[#8A4058]">{couple.lastActiveTime || 'Active now'}</span>
          </div>

          <p className="text-xs text-[#4A0420] font-medium bg-white p-2.5 rounded-xl border border-[#F4A9BF]">
            "{couple.lastActiveNote || 'Listening to our playlist & thinking of you'}"
          </p>

          <button
            onClick={handleSendHeartPulse}
            className="w-full py-2 rounded-xl bg-[#E11D48] text-white text-xs font-semibold flex items-center justify-center gap-1.5 shadow-xs cursor-pointer hover:bg-[#C81E45]"
          >
        ...
        {/* Sync Audio Mini */}
        <div className="pt-2">
          <div className="p-3.5 rounded-2xl bg-[#FFD3DE] border border-[#F4A9BF] space-y-2 text-xs">
            <audio {...musicPlayer.bind} className="hidden" />
            <span className="text-[10px] font-bold text-[#E11D48] uppercase tracking-wider flex items-center gap-1">
              <Headphones className="w-3.5 h-3.5" />
              <span>Sync Audio Room</span>
            </span>
            <p className="text-[#4A0420] font-medium flex items-center gap-1.5">
            ...
            <button
              ...
              className="w-full py-1.5 rounded-lg bg-[#FFB8CB] text-[#E11D48] font-semibold text-[11px] flex items-center justify-center gap-1 cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
            >
              {musicPlayer.isPlaying ? <Pause className="w-3 h-3" /> : <Play className="w-3 h-3 fill-[#E11D48]" />}
      ...
      <main className="flex-1 space-y-8">
        ...
        {/* Header */}
        <div>
          <span className="text-xs font-bold text-[#E11D48] uppercase tracking-wider">DAILY RITUALS</span>
          <h2 className="font-display text-4xl sm:text-5xl text-[#4A0420] font-medium mt-1">
            Together, today.
          </h2>
        ...
        <div className="p-6 rounded-3xl bg-[#FFB8CB] border border-[#F4A9BF] warm-shadow space-y-4">
          <div className="flex items-center justify-between flex-wrap gap-2">
            <h3 className="font-display text-xl font-semibold text-[#4A0420]">
              How are you feeling right now?
            </h3>
            <span className="text-xs text-[#E11D48] font-medium font-script text-base">
        ...
        <div className="p-6 sm:p-8 rounded-3xl bg-[#FFD3DE] border border-[#F4A9BF] warm-shadow-lg stationery-lines space-y-6">
          {!todayPrompt ? (
            <p className="text-xs text-[#8A4058]">Preparing today's question…</p>
          ) : (
            <>
              <div className="flex items-center justify-between border-b border-[#F4A9BF] pb-4">
                <div>
                  <span className="text-[10px] uppercase font-mono tracking-widest text-[#E11D48] font-bold">
                    DAILY QUESTION • {todayPrompt.date}
                  </span>
                  <h3 className="font-display text-2xl sm:text-3xl text-[#4A0420] font-medium mt-1">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {/* Partner A's Answer */}
                <div className="p-5 rounded-2xl bg-[#FFB8CB] border border-[#F4A9BF] space-y-2 relative">
                  <div className="flex items-center justify-between text-xs">
                    <span className="font-semibold text-[#E11D48] font-sans">{couple.nameA}'s answer</span>
                    <span className="text-[10px] text-[#8A4058]">Today</span>
                  </div>
                  <p className="font-script text-xl text-[#4A0420] leading-snug">
                    {todayPrompt.answerA ? `"${todayPrompt.answerA}"` : 'Not answered yet.'}
                  </p>
                  <div className="w-5 h-5 rounded-full bg-[#E11D48] text-white flex items-center justify-center text-[10px] absolute bottom-3 right-3 font-serif">
                    ♡
                  </div>
                </div>
                ...
                <div className="p-5 rounded-2xl bg-[#FFB8CB] border border-[#F4A9BF] space-y-2 relative">
                  <div className="flex items-center justify-between text-xs">
                    <span className="font-semibold text-[#E11D48] font-sans">{couple.nameB}'s answer</span>
                    <span className="text-[10px] text-[#8A4058]">Today</span>
                  </div>
                  <p className="font-script text-xl text-[#4A0420] leading-snug">
              ...
              <div className="pt-2 flex gap-3">
                <input
                  ...
                  className="flex-1 px-4 py-3 rounded-2xl bg-white border border-[#F4A9BF] text-xs"
                />
                <button
                  onClick={handleSaveAnswer}
                  className="px-6 py-3 rounded-2xl bg-[#E11D48] hover:bg-[#C81E45] text-white text-xs font-semibold flex items-center gap-2 cursor-pointer shadow-sm"
                >
                  <span>Add</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="p-6 rounded-3xl bg-[#FFB8CB] border border-[#F4A9BF] space-y-3">
            <h4 className="font-display text-lg font-semibold text-[#4A0420] flex items-center gap-2">
              <Flame className="w-4 h-4 text-[#E11D48]" />
              <span>Today's Micro-Rituals</span>
            </h4>
            <div className="space-y-2 text-xs">
              {RITUAL_LABELS.map((label, idx) => (
                <label key={label} className="flex items-center gap-2.5 p-2.5 rounded-xl bg-white border border-[#F4A9BF] cursor-pointer">
                  <input
                    type="checkbox"
                  ...
                  />
                  <span className="text-[#4A0420]">{label}</span>
                </label>
              ))}
            </div>
          </div>

          <div className="p-6 rounded-3xl bg-[#4A0420] text-white space-y-3">
            ...
            <div className="pt-2 flex justify-between items-center text-xs text-white/60">
              ...
              <span className="text-[#FFC145]">♡</span>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="p-6 rounded-3xl bg-[#FFB8CB] border border-[#F4A9BF] space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="text-xs text-[#E11D48] font-medium cursor-pointer hover:underline"
              >
            ...
            {isAddingGoal && (
              <form onSubmit={handleAddGoal} className="space-y-2 pb-3 border-b border-[#F4A9BF]">
                <input
                  type="text"
                  value={newGoalTitle}
                  onChange={(e) => setNewGoalTitle(e.target.value)}
                  placeholder="e.g. Read 12 books together"
                  className="w-full px-3 py-2 rounded-lg bg-white border border-[#F4A9BF] text-xs"
                />
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder="Target"
                    className="w-1/2 px-3 py-2 rounded-lg bg-white border border-[#F4A9BF] text-xs"
                  />
                  <input
                    type="text"
                    value={newGoalUnit}
                ...
                <button type="submit" className="w-full py-2 rounded-lg bg-[#E11D48] text-white text-xs font-semibold cursor-pointer">
                  Add Goal
                </button>
              </form>
            )}
            ...
            {goals.length === 0 ? (
              <p className="text-xs text-[#8A4058] italic">No shared goals yet — add something you're working toward together.</p>
            ) : (
                  ...
                  return (
                    <div key={g.id} className="text-xs">
                      <div className="flex items-center justify-between mb-1 gap-2">
                        <span className="font-medium text-[#4A0420] truncate">{g.title}</span>
                        <div className="flex items-center gap-2 flex-shrink-0">
                          <span className="text-[#8A4058]">{g.current}/{g.target} {g.unit}</span>
                          <button
                            onClick={() => updateGoal(g.id, { current: g.current + 1 })}
                            title="Log one"
                            className="w-5 h-5 rounded-full bg-[#E11D48] text-white text-[11px] font-bold flex items-center justify-center cursor-pointer hover:bg-[#C81E45]"
                          >
                            +
                          </button>
                        </div>
                      </div>
                      <div className="w-full bg-white h-1.5 rounded-full overflow-hidden border border-[#F4A9BF]">
                        <div className="bg-[#E11D48] h-full rounded-full transition-all" style={{ width: `${pct}%` }} />
                      </div>
                    </div>
                  );
                })}
          ...
          <div className="p-6 rounded-3xl bg-[#FFB8CB] border border-[#F4A9BF] space-y-3">
            ...
            {promises.length === 0 ? (
              <p className="text-xs text-[#8A4058] italic">No promises sealed yet.</p>
            ) : (
              <div className="space-y-2 max-h-40 overflow-y-auto pr-1">
                {promises.map((p) => (
                  <div key={p.id} className="p-2.5 rounded-xl bg-white border border-[#F4A9BF] text-xs">
                    <p className="text-[#4A0420] font-script text-base leading-snug">"{p.text}"</p>
                    <p className="text-[10px] text-[#8A4058] mt-1">
            ...
            <form onSubmit={handleAddPromise} className="flex gap-2 pt-1">
              <input
                ...
                className="flex-1 px-3 py-2 rounded-lg bg-white border border-[#F4A9BF] text-xs"
              />
              <button type="submit" className="px-3 py-2 rounded-lg bg-[#E11D48] text-white text-xs font-semibold cursor-pointer">
                Seal
              </button>
            </form>
          </div>
